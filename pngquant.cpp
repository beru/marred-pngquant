...
{
    const double v1 = ((const colormap_item*)ch1)->popularity;
    const double v2 = ((const colormap_item*)ch2)->popularity;
    return v1 > v2 ? 1 : -1;
}
...
static void remap_to_palette_floyd(png24_image *input_image, png8_image *output_image, const colormap *map, const double min_opaque_val, const double* edge_map, const int output_image_is_remapped)
{
    ...
    for (unsigned int row = 0; row < rows; ++row) {
        ...
        do {
            ...
            if (fs_direction) {
                thiserr[col + 2].alpha += (err.alpha * 7.0) / 16.0;
                thiserr[col + 2].r += (err.r * 7.0) / 16.0;
                thiserr[col + 2].g += (err.g * 7.0) / 16.0;
                thiserr[col + 2].b += (err.b * 7.0) / 16.0;

                nexterr[col    ].alpha += (err.alpha * 3.0) / 16.0;
                nexterr[col    ].r += (err.r * 3.0) / 16.0;
                nexterr[col    ].g += (err.g * 3.0) / 16.0;
                nexterr[col    ].b += (err.b * 3.0) / 16.0;

                nexterr[col + 1].alpha += (err.alpha * 5.0) / 16.0;
                nexterr[col + 1].r += (err.r * 5.0) / 16.0;
                nexterr[col + 1].g += (err.g * 5.0) / 16.0;
                nexterr[col + 1].b += (err.b * 5.0) / 16.0;

                nexterr[col + 2].alpha += (err.alpha       ) / 16.0;
                nexterr[col + 2].r += (err.r       ) / 16.0;
                nexterr[col + 2].g += (err.g       ) / 16.0;
                nexterr[col + 2].b += (err.b       ) / 16.0;
            } else {
                thiserr[col    ].alpha += (err.alpha * 7.0) / 16.0;
                thiserr[col    ].r += (err.r * 7.0) / 16.0;
                thiserr[col    ].g += (err.g * 7.0) / 16.0;
                thiserr[col    ].b += (err.b * 7.0) / 16.0;

                nexterr[col    ].alpha += (err.alpha       ) / 16.0;
                nexterr[col    ].r += (err.r       ) / 16.0;
                nexterr[col    ].g += (err.g       ) / 16.0;
                nexterr[col    ].b += (err.b       ) / 16.0;
                ...
                nexterr[col + 2].alpha += (err.alpha * 3.0) / 16.0;
                nexterr[col + 2].r += (err.r * 3.0) / 16.0;
                nexterr[col + 2].g += (err.g * 3.0) / 16.0;
                nexterr[col + 2].b += (err.b * 3.0) / 16.0;
            }
...
static histogram *get_histogram(const png24_image *input_image, const unsigned int reqcolors, const unsigned int speed_tradeoff, const double* importance_map)
{
    histogram *hist;
...
static void modify_alpha(png24_image *input_image, const double min_opaque_val)
{
    ...
    const unsigned int almost_opaque_val_int = almost_opaque_val*255.0;
 ...
 */
static void contrast_maps(const rgb_pixel*const apixels[], const unsigned int cols, const unsigned int rows, const double gamma, double** noiseP, double** edgesP)
{
    double* RESTRICT noise = (double*) malloc(sizeof(double)*cols*rows);
    double* RESTRICT tmp = (double*) malloc(sizeof(double)*cols*rows);
    double* RESTRICT edges = (double*) malloc(sizeof(double)*cols*rows);
    ...
    for (unsigned int j=0; j < rows; j++) {
        ...
        for (unsigned int i=0; i < cols; i++) {
            ...
            const double a = abs(prev.alpha+next.alpha - curr.alpha*2.0),
            r = abs(prev.r+next.r - curr.r*2.0),
            g = abs(prev.g+next.g - curr.g*2.0),
            b = abs(prev.b+next.b - curr.b*2.0);

            const f_pixel prevl = to_f(gamma, apixels[min(rows-1,j+1)][i]);
            const f_pixel nextl = to_f(gamma, apixels[j > 1 ? j-1 : 0][i]);

            const double a1 = abs(prevl.alpha+nextl.alpha - curr.alpha*2.0),
            r1 = abs(prevl.r+nextl.r - curr.r*2.0),
            g1 = abs(prevl.g+nextl.g - curr.g*2.0),
            b1 = abs(prevl.b+nextl.b - curr.b*2.0);

            const double horiz = max(max(a,r),max(g,b));
            const double vert = max(max(a1,r1),max(g1,b1));
            const double edge = max(horiz,vert);
            double z = edge - abs(horiz-vert)*.5;
    ...
    for (unsigned int row=0; row < height; row++) {
        ...
        for (unsigned int col=1; col < width; col++) {
            ...
            if (px != lastpixel || col == width-1) {
                ...
                while (lastcol <= col) {
                    edges[row*width + lastcol++] *= 1.0 - 2.50/neighbor_count;
                }
                lastpixel = px;
            }
        }
...
static colormap *find_best_palette(histogram *hist, int reqcolors, const double min_opaque_val, const double target_mse, int feedback_loop_trials, double *palette_error_p)
{
    ...
    do {
        ...
        if (!acolormap || total_error < least_error || (total_error <= target_mse && newmap->colors < reqcolors)) {
            ...
            reqcolors = min<unsigned int>(newmap->colors+1, reqcolors);
